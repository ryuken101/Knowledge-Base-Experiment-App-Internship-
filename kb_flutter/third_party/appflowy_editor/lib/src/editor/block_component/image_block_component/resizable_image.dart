...
class _ResizableImageState extends State<ResizableImage> {
  ...
  Widget _buildResizableImage(BuildContext context) {
    ...
    if (isBase64(src)) {
      ...
      // load local file. LOCAL PATCH (kb_flutter): a local path saved into the
      // markdown KB may not exist on this machine (or got mangled on a markdown
      // round-trip). Guard the file so it renders the error placeholder instead
      // of throwing an uncaught image-load exception.
      final file = File(src);
      if (file.existsSync()) {
